<div class="w-full h-full relative">
    ...
    <div class="relative w-full z-0 min-h-[calc(100vh)] pt-28 flex flex-col bg-cover bg-no-repeat bg-top-right overflow-hidden transition-all duration-500 max-md:ps-0 min-md:ps-48"
        ...
        <div class="relative text-dark flex flex-col gap-y-2 p-4 ps-8 pb-8 border-b-2 border-dark shrink-0">
            <p class="text-2xl font-bold">Kelola Mahasiswa</p>
            <p class="text-sm">Kelola Mahasiswa</p>
        </div>
        ...
        <div class="flex flex-row justify-between w-full flex-1 border-b-2 z-10 border-dark">
            ...
            <div class="w-full border-x-2 border-dark p-4 px-8 flex flex-col gap-y-8">
                <livewire:components.mahasiswa-table-action/>           
                <div class="flex flex-col gap-y-4">
                    <p class="text-xl font-bold text-dark border-b-2 pe-4 pb-2">Detail Mahasiswa</p>
                    <livewire:mahasiswa-table/>
                    <div class="" :class=" sidebarOpen ? 'max-w-[calc(100vw*0.8)]' : 'max-w-[calc(100vw*0.9)]'">
                    </div>  
                    <div>
                        <form id="tambahMahasiswa" class="w-full flex flex-col gap-y-8">
                            <p class="w-full text-xl font-bold text-dark border-b-2 pe-4 pb-2">NIM Mahasiswa</p>
                            <flux:input type=text description="Masukkan Nomor Induk Mahasiswa" icon="document" class="rounded-full!" model="nim" />       
                            <p class="w-full text-xl font-bold text-dark border-b-2 pe-4 pb-2">Nama Mahasiswa</p>  
                            <flux:input type=text description="Masukkan Nama Mahasiswa" icon="user" class="rounded-full!" model="name" />                        
                            <p class="w-full text-xl font-bold text-dark border-b-2 pe-4 pb-2">Kata Sandi</p>  
                            <flux:input type=text description="Tambahkan Kata Sandi Mahasiswa" icon="key" class="rounded-full!" model="password" />                        
                            <flux:button variant="primary" class="active:scale-95 active:bg-gray-200! hover:bg-primary-shadow bg-primary text-white transition-all duration-100" wire:click="tambahMahasiswa">Tambah</flux:button>
                        </form> 
                    </div>  
                </div>               
            </div>
